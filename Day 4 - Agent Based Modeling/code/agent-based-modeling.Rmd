...
agent.generator <- function(N){
  ...
  for (i in 1:N) {
    agents[[i]] <- new("agent", id=i,infected=0,
                       shape=sample(c("square","circle"), size=1, 
                                    prob=c(0.5,0.5)))
  }
  return(agents)
}
...
select.partner <- function(i, agents, H){
    ...
    if (i.shape == "circle") {
      alter.shape <- sample(c("square","circle"), size=1, 
                            prob=c(1-H,H))
    }
    else {
      alter.shape <- sample(c("square","circle"), size=1, 
...
results.matrix <- matrix(nrow=K*t*length(H.vals)*length(P.vals), 
                         ncol=5)
```
...
stats <- df %>% filter(time == t) %>% group_by(P,H) %>% 
...
ggplot(data = stats, 
       aes(x= P, y  = mean, ymin = lower, ymax = upper, color=as.factor(H))) + 
  geom_pointrange()  + labs(title="Comparing different transmission probabilities by homophily strength", y="Mean proportion infected", x="P", caption="100 trials of each transmission probability. Two groups with equal sizes.") + scale_color_viridis_d() + theme_bw()
```
